...
Kaggle est l'un des endroits où un notebook de l'atelier peut s'exécuter, aux côtés
de Colab, de votre propre machine et d'un simple exécuteur d'intégration
continue. La boîte à outils le détecte automatiquement, et plusieurs des notebooks
...
voie testée vers un notebook qui s'exécute, c'est la valeur réelle de Kaggle pour
...
| Tout ce qui nécessite une clé API que vous avez créée | Colab. Kaggle Secrets fonctionne, mais c'est le gestionnaire de Colab qui a été testé avec les notebooks. |
| Publier vos résultats | GitHub Pages. Un notebook Kaggle n'est pas un produit public citable ; le Jour 5 explique ce qui l'est. |
...
2. **Vérifiez votre numéro de téléphone.** C'est l'étape que l'on manque :
   l'accès au GPU et l'accès internet depuis les notebooks restent tous deux
...
3. Optionnel : Settings → Account → Create New API Token, si vous souhaitez
   pousser des jeux de données ou des notebooks depuis votre machine.

Total : environ cinq minutes, plus le délai du SMS.

---

## Exécuter un notebook de l'atelier sur Kaggle

Tous les notebooks de ce dépôt détectent Kaggle automatiquement.
...
3. Réglages du notebook à droite : **Internet → On**
4. Pour les clés API : Add-ons → Secrets → attachez votre clé sous le nom exact
   attendu par le notebook (`GROQ_API_KEY`, `GITHUB_TOKEN`, …)

!!! warning "L'accès internet est désactivé par défaut sur Kaggle"

    Un notebook Kaggle neuf n'a aucun accès réseau, et `pip install` échoue sur un
